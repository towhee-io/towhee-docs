...
<KatacodaBlock>pip install towhee==0.4.0</KatacodaBlock>
...
Once Towhee is installed, let's move onto a bit of prep work. Download our Towhee logo (or any other image); we'll use this to generate an embedding vector:
...
We can skip this step if we want to use url to find the image, Towhee support both ways.
...
Towhee allow users to specify the image both by local directory and remote url.

If we have downloaded the Towhee logo in the previous prep step:

<KatacodaBlock>output = embedding_pipeline('towhee.jpeg')</KatacodaBlock>

Or we can just use the url:

<KatacodaBlock>output = embedding_pipeline('https://github.com/towhee-io/towhee-docs/raw/main/docs/get-started/towhee.jpeg')</KatacodaBlock>
